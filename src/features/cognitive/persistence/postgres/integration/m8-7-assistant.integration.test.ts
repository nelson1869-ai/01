...
} from "../../../api/assistant-ai";
import {
  AssistantChatService,
  DatabaseAssistantConversationStore,
} from "../../../api/assistant-chat-service";
...
class ToolIntent implements AssistantIntentInterpreterPort {
  async interpret() {
    return {
      ...
      action: "github.contents.read" as const,
      path: "README.md",
      issueNumber: null,
      pullNumber: null,
      response: null,
...
class Composer implements AssistantResponseComposerPort {
  ...
  async composeVerified() {
    this.verifiedCalls++;
    return "I checked README.md and verified the result. AutoDo is safe automation.";
  }
}
...
describe("M8.7 assistant durable pipeline (fake Gemini/GitHub networking)", () => {
  ...
  function makeService(
    ...
    return new AssistantChatService({
      ...
      interpreter,
      composer,
      toolRunner: new DatabaseAssistantToolRunner(context.db, {
        adapter,
        verifier,
      }),
      now: () => NOW,
  ...
  it("persists conversation continuity and runs a GitHub read through grounding, policy, authorization, observation, and verification", async () => {
    ...
    const response = await makeService(adapter, composer).chat({
    ...
    const policy = await policyRepository.findPolicyDecisionByCandidateId(
      context.db,
  ...
  it("M8.8: handles transient provider recovery in live database, keeping exactly 1 turn with telemetry", async () => {
    const adapter = new FakeGitHubAdapter();
    const composer = new Composer();
    let calls = 0;
    const interpreter: AssistantIntentInterpreterPort = {
      async interpret(_msg, _ctx, options) {
        calls++;
        if (calls === 1) {
          ...
          // simulate transient failure recovered on attempt 2
        }
        options?.telemetryCollector?.record({
          stage: "assistant.intent",
          provider: "gemini",
    ...
    const response = await makeService(
      ...
      message: "Explain AutoDo.",
    });

    expect(response.status).toBe("COMPLETED");
    expect(response.telemetry).toBeDefined();
    expect(response.telemetry.ai.length).toBeGreaterThan(0);
    const turns =
      await assistantConversationRepository.findAllTurnsForConversation(
        context.db,
        response.conversationId,
